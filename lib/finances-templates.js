...
function getFinanceDiscussionQuestion(person1Finances, person2Finances, person1Name, person2Name) {
    ...
    // Check for shared fears (works with both new ranked and old boolean format)
    ...
    // NEW FORMAT: Check top 2 ranked fears
    if (p1Fears.ranked && p2Fears.ranked) {
        const p1TopFears = p1Fears.ranked.slice(0, 2).map(f => mapFearTextToKey(f.fear)).filter(Boolean);
        const p2TopFears = p2Fears.ranked.slice(0, 2).map(f => mapFearTextToKey(f.fear)).filter(Boolean);
        
        p1TopFears.forEach(p1Fear => {
            if (p2TopFears.includes(p1Fear) && FINANCIAL_FEAR_TEMPLATES[p1Fear]) {
                sharedFears.push(FINANCIAL_FEAR_TEMPLATES[p1Fear].label.toLowerCase());
            }
        });
    }
    // OLD FORMAT: Check boolean flags
    else {

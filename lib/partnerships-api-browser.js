/**
 * Partnerships API - Browser Compatible
 ...
 */
async function sendPartnerRequest(partnerId) {
  try {
    const supabase = await window.supabaseAuth.getSupabase();
    const currentUser = await window.supabaseAuth.getCurrentUser();
 ...
 */
async function getPartnership(partnerId) {
  try {
    const supabase = await window.supabaseAuth.getSupabase();
    const currentUser = await window.supabaseAuth.getCurrentUser();
    if (!currentUser) throw new Error('Not authenticated');
 ...
 */
async function acceptPartnershipRequest(partnershipId) {
  try {
    const supabase = await window.supabaseAuth.getSupabase();
    const { data, error } = await supabase
      .from('partnerships')
 ...
 */
async function declinePartnershipRequest(partnershipId) {
  try {
    const supabase = await window.supabaseAuth.getSupabase();
    const { data, error } = await supabase
      .from('partnerships')
...
/**
 * Delete a partnership (for removing connections or canceling requests)
 */
 ...
 */
async function hasAcceptedPartnership() {
  try {
...
async function getAllPartnerships() {
  try {
    ...
    return data.map(partnership => ({
      ...
      user1: profileMap[partnership.user1_id],
      user2: profileMap[partnership.user2_id],
      partner: partnership.user1_id === currentUser.id ? profileMap[partnership.user2_id] : profileMap[partnership.user1_id]
    }));
  } catch (error) {
    console.error('Error getting all partnerships:', error);
    return [];
  }
}
...
console.log('✅ Partnerships API (Browser) loaded');
